...
public class InstrumentRepository : TransactionalRepository, IInstrumentRepository
{
    public InstrumentRepository(BrokerDbContext dbContext) : base(dbContext) { }

    public IQueryable<Instrument> GetAllInstruments()
        => DbContext.Instruments.AsQueryable();
    ...
    public async Task<OwnedInstrument?> GetOwnedInstrument(int accountId, int instrumentId)
        => await DbContext.OwnedInstruments.Where(x => x.AccountId == accountId && x.InstrumentId == instrumentId).FirstOrDefaultAsync();
