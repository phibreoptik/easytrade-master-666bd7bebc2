...
public class BalanceService : IBalanceService
{
    ...
    public BalanceService(IBalanceRepository balanceRepository, ILogger<BalanceService> logger)
    {
    ...
    {
        _logger.LogInformation("Modify balance with action type [{action}], amount [{amount}], account ID [{id}]", actionType, amount, accountId);
        ...
        var balanceHistory = new BalanceHistory(accountId, balance.Value, balanceDifference, actionType);

        switch(actionType)
        {
            case ActionType.Withdraw:
                balance.Withdraw(amount); break;
            case ActionType.Deposit:
                balance.Deposit(amount); break;
            default:
